...
const AddTeamModal = ({ onClose, onAddTeam }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 transition-opacity" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-md m-4 transform transition-all animate-fade-in-scale" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-xl font-semibold mb-4 text-slate-900 dark:text-white">افزودن تیم جدید</h3>
                <form onSubmit={handleSubmit}>
                    <label htmlFor="teamName" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                        نام تیم
                    </label>
                    <input
                        id="teamName"
